...
func (r *CommentRepository) GetListByPostId(post_id int) ([]*entities.Comment, error) {
	...
	if err := r.Conn.Table("comments").Select("comments.id, comments.user_id, comments.post_id, users.name as user_name, comments.body, comments.created_at, comments.updated_at").Where("comments.deleted_at IS NULL").Joins("JOIN users ON comments.user_id = users.id").Where("comments.post_id = ?", post_id).Order("comments.id ASC").Scan(&comments).Error; err != nil {
		return nil, err
	}
	return comments, nil
}
